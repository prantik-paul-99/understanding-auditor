...
Format with clear sections. Keep it under 500 words. Make every sentence earn its place.
  ...
  async function generateStressTest({ concept, background, gapAnalysis }) {
    const system = `You are a rigorous examiner who designs stress tests to distinguish surface-level knowledge from deep understanding. Your questions target the specific weak points of this specific learner. You always return valid JSON and nothing else.`;
...
[
  ...
  async function evaluateStressTest({ concept, questions, answers }) {
    ...
    const qa = questions.map((q, i) => `Q${i+1} [${q.type}]: ${q.question}\nAnswer: ${answers[i] || "(no answer)"}`).join("\n\n");
...
Be honest. Don't pad with encouragement.
  ...
  async function scoreFinalTeachBack({ concept, background, finalTeachBack, gapAnalysis }) {
    const system = `You are a rigorous understanding evaluator. You score how well someone has internalized a concept based on their final explanation. You are calibrated, not generous. A 90+ means they genuinely understand it. Return valid JSON only.`;
...
Evaluate their final explanation and return ONLY this JSON, no markdown, no preamble:
{
  "score": <integer 0-100>,
...
${finalTeachBack}
    ...
    const prompts = {
      ...
      mindmap: {
        system: `You create clear, well-structured concept maps in HTML. Use divs and CSS to visually represent a concept and its connected ideas. Make it visually appealing using a dark theme with gold and teal accents. Return only the HTML/CSS snippet for the map — no full page, just the content div and a <style> block.`,
      ...
      infographic: {
        ...
        user: `${context}\n\nCreate an infographic about "${concept}" tailored to this learner. Include: a headline definition, 3-4 key facts or stats (you can use general knowledge), the learner's top 2 gaps as "watch out" callouts, and a "bottom line" summary. Make it visually rich — use colored sections, emoji icons, and typographic hierarchy.`,
        tokens: 1400
      },
      ...
      flashcards: {
        ...
        user: `${context}\n\nCreate 8 flashcards for "${concept}" targeting this learner's specific gaps. Mix question types: definition, application, "what's the difference between X and Y", and "what happens when...". Style them with dark backgrounds, gold card fronts, teal card backs. Show both sides stacked for each card. Use the .flashcard, .flashcard-front, .flashcard-back classes already defined, or add your own styles.`,
        tokens: 1200
      },
      ...
      slides: {
        ...
        user: `${context}\n\nCreate a 6-slide mini-lecture on "${concept}" for someone with this background. Slides: 1) Hook/why it matters, 2) Core definition, 3) How it works, 4) Common misconceptions (from the gaps), 5) Real-world example, 6) Key takeaway. Style each slide with a dark background, numbered, with large headings and concise bullets. Make it look like something you'd actually present.`,
        tokens: 1400
      },
      ...
      explainer: {
        ...
        user: `${context}\n\nWrite a 400-500 word explainer article on "${concept}" at the level appropriate for this learner's background. Structure: engaging intro, core explanation (addressing their gaps), one concrete example, and a memorable closing line. Use headers. Write like a smart friend who actually knows the subject — not a textbook.`,
        tokens: 1200
      },
  ...
  // ── Public API ────────────────────────────────────────────────────────────
  return { analyzeGaps, explainConcept, generateStressTest, evaluateStressTest, scoreFinalTeachBack, generateArtifact, renderText };
